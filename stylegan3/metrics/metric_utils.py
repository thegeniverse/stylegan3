...
class FeatureStats:
    ...
    def is_full(self):
        return (self.max_items
                is not None) and (self.num_items >= self.max_items)
...
def compute_feature_stats_for_dataset(opts,
    ...
    if opts.cache:
        ...
        cache_file = dnnlib.make_cache_dir_path('gan-metrics',
                                                cache_tag + '.pkl')
    ...
    for images, _labels in torch.utils.data.DataLoader(dataset=dataset,
                                                       sampler=item_subset,
