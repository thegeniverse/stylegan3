...
def stream_source_images(
        ...
        seed,
        device,
        data_loader_kwargs=None):  # => num_images, image_size, image_iter
    ...
    if ext == 'pkl':
        if num is None:
            raise click.ClickException(
                '--num is required when --source points to network pickle')
        ...
        dataset_obj = dataset.ImageFolderDataset(path=source,
                                                 max_size=num,
                                                 random_seed=seed)
        if num is not None and num != len(dataset_obj):
            raise click.ClickException(
                f'--source contains fewer than {num} images')
...
def stats(source, num, seed, device=torch.device('cuda')):
    ...
    num_images, _image_size, image_iter = stream_source_images(source=source,
                                                               num=num,
                                                               seed=seed,
                                                               device=device)
...
@click.option('--dest',
              ...
              required=True)
@click.option('--mean',
              help='Dataset mean for whitening',
              metavar='FLOAT',
              type=float,
              required=True)
@click.option('--std',
...
def calc(source,
    ...
    torch.multiprocessing.set_start_method('spawn')
    num_images, image_size, image_iter = stream_source_images(source=source,
                                                              num=num,
                                                              seed=seed,
                                                              device=device)
    ...
    window = torch.kaiser_window(image_size,
                                 periodic=False,
                                 beta=beta,
                                 device=device)
    ...
    spectrum = torch.zeros([spectrum_size, spectrum_size],
                           dtype=torch.float64,
                           device=device)
    for image in tqdm.tqdm(image_iter, total=num_images):
        image = (image.to(torch.float64) - mean) / std
        image = torch.nn.functional.pad(image * window,
                                        [0, padding, 0, padding])
        spectrum += torch.fft.fftn(image,
                                   dim=[2, 3]).abs().square().mean(dim=[0, 1])
    spectrum /= num_images
...
def heatmap(npz_file, save, smooth, dpi):
    ...
    freqs = np.linspace(-0.5, 0.5, num=hmap.shape[0],
                        endpoint=True) * image_size
...
def slices(npz_files, save, dpi, smooth):
    ...
    for c in cases:
        c.hmap, c.image_size = construct_heatmap(npz_file=c.npz_file,
                                                 smooth=smooth)
    ...
    if any(c.image_size != image_size or c.hmap.shape[0] != hmap_size
           for c in cases):
    ...
    freqs0 = np.linspace(0,
                         image_size / 2,
                         num=(hmap_size // 2 + 1),
                         endpoint=True)
    freqs45 = np.linspace(0,
                          image_size / np.sqrt(2),
                          num=(hmap_size // 2 + 1),
                          endpoint=True)
    xticks0 = np.linspace(freqs0[0], freqs0[-1], num=9, endpoint=True)
    xticks45 = np.round(
        np.linspace(freqs45[0], freqs45[-1], num=9, endpoint=True))
